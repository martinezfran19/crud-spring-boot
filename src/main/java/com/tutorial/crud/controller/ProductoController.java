...
public class ProductoController {
    ...
    public ResponseEntity<?> getByName(@PathVariable("nombre") String nombre) {
        if (!productoService.existByNommbre(nombre)) {
            return new ResponseEntity<>(new Mensaje("El producto solicitado no existe"), HttpStatus.NOT_FOUND);
        }
    ...
    public ResponseEntity<Mensaje> create(@RequestBody ProductDto productDto) {
        ...
        if (productoService.existByNommbre(productDto.getNombre())) {
            return new ResponseEntity<Mensaje>(new Mensaje("El El producto ya se encuentra registrado"), HttpStatus.BAD_REQUEST);
        }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Mensaje> update(@PathVariable("id") int id, @RequestBody ProductDto productDto) {
        if (!productoService.existsById(id)) {
